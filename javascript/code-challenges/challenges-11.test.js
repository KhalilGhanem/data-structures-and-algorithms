...
function transformToLis(obj){
  let listarr=[];
    let okey=Object.entries(obj).forEach(item =>{
      listarr.push(`<li>${item[0]}: ${item[1]}</li>`);
      console.log(item);
    })
...
const divisibleByFiveTwoToThePower = (input) => {
  let arr=input.map(item =>{
    return item.filter(value=>value%5==0 && typeof value =='number' ).map(item =>Math.pow(2,item));
  });
...
let findMaleAndFemale = (data) => {
  let arr=data.filter(item =>item.gender =='female'|| item.gender =='male');
  console.log(arr);
  let names=[]
  arr.forEach(item=>{
...
let findShortest = (data) => {
  let charh=Number(data[0].height);
  let charname='';
console.log(charh);
  data.forEach(item =>{
    console.log(item.height);
    if(charh>=Number(item.height)){
      charh=item.height;
      charname=item.name;
    }
  })
